...
namespace TesisProj.Areas.Simulaciones.Controllers
{
    public class TStudentController : Controller
    {
        ...
        public ActionResult Index(int ProyectoId, int ParametroId)
        {
            ...
            List<ListField> lista = db.ListFields.Where(p => p.Modelo == "TStudent").ToList();
            ModeloSimulacion modelo = new ModeloSimulacion("TStudent", lista);
        ...
        public ActionResult Index(_TStudent u)
        {
            ...
            TProjContext db = new TProjContext();
            List<ListField> lista = db.ListFields.Where(p => p.Modelo == "TStudent").ToList();
            ModeloSimulacion modelo = new ModeloSimulacion("TStudent", u.ParamsIN[0].valorD, u.ParamsIN[1].valorD, u.ParamsIN[2].valorD, 0,lista);
        ...
        public FileContentResult getImg(int Id)
        {
            TProjContext db = new TProjContext();
            List<ListField> lista = db.ListFields.Where(p => p.Modelo == "TStudent").ToList();
            ModeloSimulacion m = new ModeloSimulacion("TStudent", lista);
            byte[] byteArray;
            if (Id == -1)
