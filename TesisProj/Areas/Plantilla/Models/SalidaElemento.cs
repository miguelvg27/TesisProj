...
namespace TesisProj.Areas.Plantilla.Models
{
    ...
    public class SalidaElemento : DbObject, IValidatableObject
    {
        ...
        public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
        {
            ...
            {
                if (context.SalidaElementos.Any(s => s.Nombre == this.Nombre && s.IdPlantillaElemento == this.IdPlantillaElemento && (this.Id > 0 ? s.Id != this.Id : true)))
                {
                    yield return new ValidationResult("Ya existe un registro con el mismo nombre en la misma plantilla.", new string[] { "Nombre" });
                }

                if (context.SalidaElementos.Any(s => s.Secuencia == this.Secuencia && s.IdPlantillaElemento == this.IdPlantillaElemento && (this.Id > 0 ? s.Id != this.Id : true)))
                {
