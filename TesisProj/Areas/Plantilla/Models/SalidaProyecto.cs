...
namespace TesisProj.Areas.Plantilla.Models
{
    ...
    public class SalidaProyecto : DbObject, IValidatableObject
    {
        ...
        public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
        {
            ...
            {
                if (context.SalidaProyectos.Any(s => s.Nombre == this.Nombre && s.IdPlantillaProyecto == this.IdPlantillaProyecto && (this.Id > 0 ? s.Id != this.Id : true)))
                {
                    yield return new ValidationResult("Ya existe un registro con el mismo nombre en la misma plantilla.", new string[] { "Nombre" });
                }

                if (context.SalidaProyectos.Any(s => s.Secuencia == this.Secuencia && s.IdPlantillaProyecto == this.IdPlantillaProyecto && (this.Id > 0 ? s.Id != this.Id : true)))
                {
                ...
                {
                    formulas.AddRange(context.Formulas.Where(f => f.IdPlantillaElemento == elemento.Id).ToList());
                }
