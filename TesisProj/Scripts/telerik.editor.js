    ...
    function attr(node) {
        ...
        for (var i = 0, l = attributes.length; i < l; i++) {
            ...
            if (attribute.specified ||
                ...
                (name == 'type' && attribute.nodeValue == 'text') ||
                (name == 'style' && attribute.nodeValue != ''))
        ...
        for (var i = 0, l = specifiedAttributes.length; i < l; i++) {
            ...
            if (attributeName == 'style') {
                ...
                for (var cssIndex = 0, len = css.length; cssIndex < len; cssIndex++) {
                    ...
                    if (pair.length) {
                        ...
                        if (property.indexOf('font') >= 0) {
                            value = value.replace(quoteRe, "'").replace("/normal", "");
                        }

                        serializedValue.push(property);
                ...
                serializedValue.push(node.getAttribute(attributeName, 2));
            } else {
                serializedValue.push(fillAttrs[attributeName] ? attributeName : attributeValue);
            }
            
            if (attributeName == "style" && serializedValue.length == 0) {
                continue;
            }
            ...
            result.push('="');
            result.push(serializedValue.join(""));
            result.push('"');
        }
    }
...
function Clipboard (editor) {
    ...
    this.paste = function (html, clean) {
        ...
        if (dom.name(editor.body.lastChild) == "table") {
            var p = editor.document.createElement("p");
            p.innerHTML = "<br _moz_dirty />";

            editor.body.appendChild(p);
...
function LinkCommand(options) {
    ...
    this.exec = function () {
        ...
        var localization = self.editor.localization;
        ...
        var dialog = $t.window.create($.extend({}, this.editor.dialogOptions, {
            ...
            html: new $.telerik.stringBuilder()
                .cat('<div class="t-editor-dialog">')
                    .cat('<ol>')
                        .cat('<li class="t-form-text-row"><label for="t-editor-link-url">' + localization.webAddress + '</label><input type="text" class="t-input" id="t-editor-link-url"/></li>')
                        ...
                        .cat('<li class="t-form-text-row"><label for="t-editor-link-title">' + localization.tooltip + '</label><input type="text" class="t-input" id="t-editor-link-title"/></li>')
                        .cat('<li class="t-form-checkbox-row"><input type="checkbox" id="t-editor-link-target"/><label for="t-editor-link-target">' + localization.openInNewWindow + '</label></li>')
                    .cat('</ol>')
                    .cat('<div class="t-button-wrapper">')
                        .cat('<button class="t-dialog-insert t-button">' + localization.insert + '</button>')
                        .cat('&nbsp;' + localization.or + '&nbsp;')
                        .cat('<a href="#" class="t-dialog-close t-link">' + localization.close + '</a>')
                    .cat('</div>')
                .cat('</div>')
                .string(),
            onClose: close
...
function ImageCommand(options) {
    ...
    this.exec = function () {
        ...
        var dialog = $t.window.create($.extend({ width: 750 }, this.editor.dialogOptions, {
            title: localization.insertImage,
            html: new $.telerik.stringBuilder()
                        .cat('<div class="t-editor-dialog">')                        
                            .catIf('<div class="t-image-browser"></div>', showBrowser)
                            .cat('<ol>')
                                .cat('<li class="t-form-text-row"><label for="t-editor-image-url">' + localization.webAddress + '</label><input type="text" class="t-input" id="t-editor-image-url"/></li>')
                                .cat('<li class="t-form-text-row"><label for="t-editor-image-title">' + localization.tooltip + '</label><input type="text" class="t-input" id="t-editor-image-title"/></li>')
                            .cat('</ol>')
                            .cat('<div class="t-button-wrapper">')
                                .cat('<button class="t-dialog-insert t-button">' + localization.insert + '</button>')
                                .cat('&nbsp;' + localization.or + '&nbsp;')
    ...
    document.write(
        new $t.stringBuilder()
            ...
            .cat('<style type="text/css">')
                .cat('html,body{padding:0;margin:0;height:100%;min-height:100%;font-family:Verdana,Geneva,sans-serif;background:#fff;}')
                ...
                .cat('ul,ol{padding-left:30px;margin-left:30px;}')
                .cat('a{color:#00a}')
