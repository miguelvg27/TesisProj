(function ($) {
            ...
            if (grid.editing.mode == 'InCell') {
                ...
                grid.editCell = function (td) {
                    ...
                    if (grid.valid && (column && !column.readonly)) {
                        ...
                        if (grid.editing.insertRowPosition == "bottom" && td.closest("tr").hasClass("t-grid-new-row")) {
                            dataItem = grid.dataItem(tr) || grid.changeLog.get(index);
                        } else {
                            dataItem = grid.changeLog.get(index, td.closest("tr").hasClass("t-grid-new-row")) || grid.dataItem(tr);
                        }
    ...
    $t.grid.ChangeLog = function (size, insertRowPosition) {
        ...
        this.get = function (index, isNew) {
            isNew = typeof isNew === "undefined" ? true : isNew;
            var inserted = this.inserted[index],
                inserterCount = insertRowPosition == "bottom" ? 0 : this.inserted.length;

            if (isNew && this.inserted[index]) {
                return inserted;
            }

            return this.updated[index - inserterCount];
        }
