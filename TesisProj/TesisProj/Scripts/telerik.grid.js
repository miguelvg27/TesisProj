(function ($, undefined) {
    ...
    $t.grid.prototype = {
        ...
        rowClick: function (e) {
            var $target = $(e.target);
            if (!$target.is(':button,a,.t-delete,input,select,textarea,option,a>.t-icon')) {
                e.stopPropagation();
        ...
        hideColumn: function(column) {
            ...
            if(that.scrollable) {
                $("col:not(.t-group-col,.t-hierarchy-col)", that.$footer).eq(columnIndex).remove();
            ...
            tables = $(">table,>.t-grid-header table,>.t-grid-content>table, >.t-grid-footer>.t-grid-footer-wrap>table", that.element);            
            if(tableWidth) {            
        ...
        showColumn: function(column) {
            ...
            if(that.scrollable) {
                cols = $("col:not(.t-group-col,.t-hierarchy-col)", that.$tbody.prev());
                normalizeCols($("col:not(.t-group-col,.t-hierarchy-col)", that.$footer), columns, columnIndex);
                normalizeCols(cols, columns, columnIndex);
            }
